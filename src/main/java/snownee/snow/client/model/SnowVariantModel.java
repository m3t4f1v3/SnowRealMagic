...
public class SnowVariantModel extends BakedModelWrapper<BakedModel> {
	...
	public List<BakedQuad> getQuads(
		...
		if (modelData == null) {
			return originalModel.getQuads(state, side, randomSource, modelData, renderType);
		}
		BakedModel model = originalModel;
		if (SnowClientConfig.snowVariants && (modelData == USE_SNOW_VARIANT || modelData.has(SnowBlockEntity.OPTIONS))) {
			model = variantModel;
		}
		return model.getQuads(state, side, randomSource, modelData, renderType);
	}
